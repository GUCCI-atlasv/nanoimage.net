...
import { getPageMeta, PRIVACY_META } from '@/lib/server-i18n'
...
export async function generateMetadata(
  ...
  const { title, description } = getPageMeta(PRIVACY_META, lang)
  const canonicalUrl = `${BASE}/${lang}/privacy-policy`
  return {
    title,
    description,
    alternates: buildAlternates(canonicalUrl, '/privacy-policy'),
